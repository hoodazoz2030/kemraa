...
export default function DashboardPage() {
  ...
  const PERIODS = [
    { key: "day", label: t("dash.today") },
    { key: "week", label: t("dash.week") },
    { key: "month", label: t("dash.month") },
  ];

  const finance = useQuery({ queryKey: ["finance-summary", period], queryFn: () => partnerFinanceApi.summary(period) });
  const analytics = useQuery({ queryKey: ["analytics-overview", period], queryFn: () => partnerAnalyticsApi.overview(period) });
  const bookings = useQuery({ queryKey: ["bookings-list"], queryFn: () => partnerBookingsApi.list() });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-3xl font-bold text-kemraa-gold tracking-wider">
            {t("dash.welcome")}{user?.organization?.displayName ? `, ${user.organization.displayName}` : ""}
          </h1>
          <p className="text-sm text-kemraa-text/60 mt-1">{t("dash.subtitle")}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard icon={DollarSign} label={t("dash.gross")} value={finance.isLoading ? "—" : `EGP ${formatMinor(finance.data?.gross?.minor ?? 0)}`} />
        <KpiCard icon={TrendingUp} label={t("dash.commission")} value={finance.isLoading ? "—" : `EGP ${formatMinor(finance.data?.commission?.minor ?? 0)}`} color="text-kemraa-text" />
        <KpiCard icon={Activity} label={t("dash.net")} value={finance.isLoading ? "—" : `EGP ${formatMinor(finance.data?.net?.minor ?? 0)}`} color="text-green-400" />
        <KpiCard icon={Calendar} label={t("dash.bookings")} value={analytics.isLoading ? "—" : analytics.data?.counts?.total ?? 0} color="text-kemraa-gold" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-gradient-to-br from-kemraa-darkAlt to-kemraa-dark border border-kemraa-goldDark/20 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-kemraa-gold mb-4 tracking-wider">{t("dash.status")}</h2>
          {analytics.isLoading ? (
            <div className="text-kemraa-text/60">Loading...</div>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              <StatusRow icon={Clock} label={t("dash.new")} value={analytics.data?.counts?.new ?? 0} color="text-blue-400" />
              <StatusRow icon={CheckCircle} label={t("dash.confirmed")} value={analytics.data?.counts?.confirmed ?? 0} color="text-green-400" />
              <StatusRow icon={CheckCircle} label={t("dash.completed")} value={analytics.data?.counts?.completed ?? 0} color="text-emerald-400" />
              <StatusRow icon={XCircle} label={t("dash.cancelled")} value={analytics.data?.counts?.cancelled ?? 0} color="text-red-400" />
            </div>
          )}
        </div>

        <div className="bg-gradient-to-br from-kemraa-darkAlt to-kemraa-dark border border-kemraa-goldDark/20 rounded-xl p-6">
          <h2 className="text-lg font-semibold text-kemraa-gold mb-4 tracking-wider">{t("dash.recent")}</h2>
          {bookings.isLoading ? (
            <div className="text-kemraa-text/60">Loading...</div>
          ) : bookings.data?.items?.length === 0 ? (
            <div className="text-kemraa-text/60 py-8 text-center">{t("dash.noBookings")}</div>
          ) : (
