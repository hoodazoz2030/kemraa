...
export class OtpService {
  ...
  // In-memory store for local/sandbox. Replace with Redis in production.
  ...
  async generate(identifier: string, channel: string): Promise<string> {
    ...
    if (channel === "EMAIL" && identifier.includes("@")) {
      const result = await this.resend.sendOtp(identifier, code, this.config.otpTtlSeconds);
      if (result.mode === "console") {
        this.logger.log(`[OTP][EMAIL-CONSOLE] ${identifier} -> ${code} (expires in ${this.config.otpTtlSeconds}s)`);
      }
    } else if (channel === "SMS") {
      // TODO: integrate SMS provider (Twilio/MessageBird)
      this.logger.log(`[OTP][SMS] ${identifier} -> ${code} (expires in ${this.config.otpTtlSeconds}s)`);
    } else {
      // Fallback to console
      this.logger.log(`[OTP][${channel}] ${identifier} -> ${code} (expires in ${this.config.otpTtlSeconds}s)`);
    }
  ...
  async verify(identifier: string, channel: string, code: string): Promise<boolean> {
    ...
    if (entry.attempts >= 5) {
      this.store.delete(this.key(identifier, channel));
      return false; // brute-force protection
    }
    entry.attempts++;
